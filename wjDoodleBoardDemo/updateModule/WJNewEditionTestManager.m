...
#import "WJAppStoreInfoModel.h"

@interface WJNewEditionTestManager ()
//本地info文件
@property (strong,nonatomic) NSDictionary *infoDict;

@end

@implementation WJNewEditionTestManager
...
+ (instancetype)shareManager {
    static WJNewEditionTestManager *instance;
...
+(void)checkNewEditionWithAppID:(NSString *)appID CustomAlert:(CheckVersionBlock)checkVersionBlock {
    [[self shareManager] getAppStoreVersion:appID sucess:^(WJAppStoreInfoModel *model) {
        if(checkVersionBlock)checkVersionBlock(model);
    }];
}

- (void)checkNewVersion:(NSString *)appID ctrl:(UIViewController *)containCtrl {
    [self getAppStoreVersion:appID sucess:^(WJAppStoreInfoModel *model) {
...
#pragma mark - 立即升级
- (void)updateRightNow:(WJAppStoreInfoModel *)model {
...
- (void)getAppStoreVersion:(NSString *)appID sucess:(void(^)(WJAppStoreInfoModel *))update {
    ...
    [self getAppStoreInfo:appID success:^(NSDictionary *respDict) {
        ...
        if (resultCount == 1) {
            ...
            //字典转模型
            WJAppStoreInfoModel *model = [[WJAppStoreInfoModel alloc] init];
